...
class AuthorDetailsControllerTest {
  ...
  void shouldGetAuthorDetailsWithDocuments() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/v1/authors/1").with(csrf()))
  ...
  void shouldReturnUnprocessableEntityWhenPathAndBodyIdMismatch() throws Exception {
    ...
    AuthorUpdateDTO authorUpdateDTO =
        new AuthorUpdateDTO(2L, "John", "Doe", new ArrayList<>()); // ID mismatch
  ...
  void shouldReturnBadRequestForInvalidAuthorUpdateDTO() throws Exception {
    // Arrange
    AuthorCreateDTO invalidAuthorUpdateDTO =
        new AuthorCreateDTO(2L, "John", "Doe", null); // Invalid data
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.errorDetails.documents").value("List of document IDs should be provided"));
  }
  ...
  void shouldHandleResourceNotFound() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(delete("/api/v1/authors/1").with(csrf()))
