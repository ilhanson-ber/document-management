...
class ValidationUtilsTest {
  ...
  void shouldPassValidationWhenAllEntitiesExist() {
    List<MockEntity> entitiesFromRepo =
        List.of(new MockEntity(1L), new MockEntity(2L), new MockEntity(3L));
    ...
    ValidationUtils.validateEntitiesExistOrThrow(
        entitiesFromRepo, entitiesFromClient, "MockEntity");
  }
  ...
  void shouldThrowResourceNotFoundExceptionWhenSomeEntitiesAreMissing() {
    ...
    // This should throw ResourceNotFoundException because entity with ID 3 is missing in the
    // repository
    assertThatThrownBy(
            () ->
                ValidationUtils.validateEntitiesExistOrThrow(
  ...
  void shouldThrowResourceNotFoundExceptionWhenAllEntitiesAreMissing() {
    ...
    // This should throw ResourceNotFoundException because entities with IDs 2 and 3 are missing in
    // the repository
    assertThatThrownBy(
            () ->
                ValidationUtils.validateEntitiesExistOrThrow(
