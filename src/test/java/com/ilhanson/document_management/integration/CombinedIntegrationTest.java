...
public class CombinedIntegrationTest {
  ...
  public void completeIntegrationTest() {
    ...
    AuthorCreateDTO newAuthor =
        new AuthorCreateDTO(
            null,
            "John",
            "Doe",
            List.of(new IdInputDTO(doc1.getId()), new IdInputDTO(doc2.getId())));
    AuthorDetailsDTO createdAuthor =
        webTestClient
            .post()
            .uri("/api/v1/authors")
            .headers(httpHeaders -> httpHeaders.setBearerAuth(token))
    ...
    assertThat(createdAuthor.getDocuments())
        .extracting("id")
    ...
    DocumentCreateDTO newDocument =
        new DocumentCreateDTO(
            null, "New Doc", "This is a new document", new ArrayList<>(), new ArrayList<>());
    DocumentDetailsDTO createdDocument =
        webTestClient
            .post()
            .uri("/api/v1/documents")
            .headers(httpHeaders -> httpHeaders.setBearerAuth(token))
    ...
    // Step 4: Add the new author and document id 1 as references to this document using update,
    // using the token
    DocumentUpdateDTO updatedDocument =
        new DocumentUpdateDTO(
            createdDocument.getId(),
            createdDocument.getTitle(),
            createdDocument.getBody(),
            List.of(new IdInputDTO(doc1.getId())),
            List.of(new IdInputDTO(createdAuthor.getId())));
    webTestClient
        .put()
        ...
        .consumeWith(
            response -> {
              ...
              assertThat(updated.getReferences())
                  .hasSize(1)
                  .extracting("id")
                  .containsExactly(doc1.getId());
              assertThat(updated.getAuthors())
                  .hasSize(1)
                  .extracting("id")
                  .containsExactly(createdAuthor.getId());
            });
    ...
    webTestClient
        ...
        .consumeWith(
            response -> {
              ...
              assertThat(author.getDocuments())
                  .hasSize(1)
                  .extracting("id")
                  .containsExactly(doc1.getId());
            });
