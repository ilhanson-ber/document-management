...
public class ValidationUtils {
  ...
  public static <T extends Identifiable> void validateEntitiesExistOrThrow(
      List<T> entitiesFromRepo, Set<T> entitiesFromClient, String entityName) {
    Set<Long> idsFromRepo = entitiesFromRepo.stream().map(T::getId).collect(Collectors.toSet());

    Set<Long> missingIds = entitiesFromClient.stream().map(T::getId).collect(Collectors.toSet());
