...
public class DocumentService {
  ...
  private void associateAuthorsWithDocument(Set<Author> authorsFromClient, Document document) {
    ...
    List<Author> authorsFromRepo =
        authorRepository.findAllById(CollectionUtils.extractIds(authorsFromClient));
  ...
  private void associateReferencesWithDocument(
      Set<Document> referencesFromClient, Document document) {
    log.debug(
        "Associating references {} with document: {}", referencesFromClient, document.getId());
    List<Document> referencesFromRepo =
        documentRepository.findAllById(CollectionUtils.extractIds(referencesFromClient));
  ...
  private void validateNoSelfReferencingOrThrow(
      Set<Document> referencesFromClient, Document document) {
    if (referencesFromClient.contains(document)) {
      ...
      throw new AssociationConflictException(
          MessageFormat.format(
              "Document can not reference itself. Remove ID({0}) from the reference list",
              document.getId()));
    }
  }

  private void validateRequestedAuthorsExistsOrThrow(
      List<Author> authorsFromRepo, Set<Author> authorsFromClient) {
    log.debug("Validating author existence for document");
    ValidationUtils.validateEntitiesExistOrThrow(authorsFromRepo, authorsFromClient, "Author(s)");
  }

  private void validateRequestedReferencesExistsOrThrow(
      List<Document> referencesFromRepo, Set<Document> referencesFromClient) {
    log.debug("Validating reference existence for document");
    ValidationUtils.validateEntitiesExistOrThrow(
        referencesFromRepo, referencesFromClient, "Document(s)");
  }
