...
public class AuthorEventProducer {
    ...
    public void sendAuthorUpdatedEvent(AuthorDetailsDTO authorDetailsDTO) {
        ...
        try {
            ...
            log.info("Successfully sent Author Updated Event for Author ID: {}", authorDetailsDTO.getId());
        } catch (Exception e) {
            log.error("Failed to send Author Updated Event for Author ID: {}", authorDetailsDTO.getId(), e);
        }
    }
